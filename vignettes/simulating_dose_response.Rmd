...
results <- vector(length = nreps)
for(i in 1:nreps) {
  result <- comparison_test(is_between, lux_1, lux_2, nindiv, population_df)
...
```{r}
nindiv <- 20
results <- map_dbl(1:nreps, ~comparison_test(is_between, lux_1, lux_2, nindiv, population_df))
mean(results)
```
...
nindiv <- 20
nreps <- 30
results_b <- map_dbl(1:nreps, ~comparison_test(TRUE, lux_1, lux_2, nindiv, population_df))
mean(results_b)

results_w <- map_dbl(1:nreps, ~comparison_test(FALSE, lux_1, lux_2, nindiv, population_df))
mean(results_w)
```
